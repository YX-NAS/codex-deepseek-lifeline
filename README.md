...
Turn on DeepSeek with the recommended model:

```bash
~/.codex/codex-deepseek-switch.sh on deepseek-v4-pro
```

The script prompts for your DeepSeek API key, sets the macOS launch environment for Codex Desktop, updates `~/.codex/config.toml`, restarts the local proxy, and writes logs to `~/.codex/deepseek-proxy.log`. You do not need multiple terminals, and you should not put the API key in source code.
